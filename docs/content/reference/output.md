...
describes what users get. Pick a format with `-o`, or let stapi choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
stapi <command> -o table   # aligned columns for reading
stapi <command> -o jsonl   # one JSON object per line, for piping
stapi <command> -o json    # a single JSON array
stapi <command> -o csv     # spreadsheet friendly
stapi <command> -o tsv     # tab-separated
stapi <command> -o url     # just the URL column
stapi <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
stapi <command> --fields id,title,url
```
...
```bash
stapi <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
stapi <command>            # a table, because this is a terminal
stapi <command> | wc -l    # JSONL, because this is a pipe
```
